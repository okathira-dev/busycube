...
export function mount(root: PocRoot): () => void {
  ...
  const runPayment = async (mode: PaymentMode) => {
    ...
    try {
      const payment = new PaymentRequestConstructor(
        ...
        {
          id: `busycube-${mode}-${crypto.randomUUID()}`,
          total: {
            label: "Busycube fictional balance",
            amount: { currency: "BCU", value: "1.00" },
          },
        },
      );
      ...
      if (response.methodName !== method) {
        ...
        renderStage("Busycube handlerを選択してください。");
        return;
      }
      ...
      if (details.outcome === "approved" && details.accepted === true) {
        ...
        render("Busycube handlerの承認とcomplete(success)を確認しました。");
        return;
      }
      ...
      if (details.outcome === "declined" && details.accepted === false) {
        ...
        render("Busycube handlerの意図的拒否とcomplete(fail)を確認しました。");
        return;
      }
  ...
  const onWorkerMessage = (event: MessageEvent<unknown>) => {
    ...
    if (message.channel !== "busycube-payment") return;
    if (message.type === "handler-event") {
      ...
      if (trusted && methodName === method) {
        renderStage(
          `Busycube handlerのtrusted eventを受信しました。handler windowで操作を選べます。attempt=${attempt}`,
        );
      }
      return;
    }
  ...
  const reset = async () => {
    for (const candidate of await navigator.serviceWorker.getRegistrations()) {
      if (candidate.scope.includes("/poc/payment/")) {
