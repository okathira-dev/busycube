...
document
  ...
  ?.addEventListener("click", async () => {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: "Busycube PoC external control",
      artist: "POC-031",
    });
    for (const action of mediaSessionActionNames) {
      try {
...
document.querySelector("#run-unicode")?.addEventListener("click", async () => {
  ...
  await Promise.all(
    expressions.map((element) =>
      document.fonts.load('2rem "Busycube Unifont"', element.textContent ?? ""),
    ),
  );
  ...
  const fontLoaded = document.fonts.check('2rem "Busycube Unifont"');
  report(
