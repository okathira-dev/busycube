...
Each fixed 640×360 VP9 WebM has an 8 second timeline: a two-word lowercase text key from 0–4 seconds and a slot-specific QR token from 4–8 seconds. The source, keys, QR encoder, generation steps, checksums, and segment ranges are tracked by `scripts/generate-busycube-s700-fixtures.mjs` and `assets/generation-manifest.json`.
