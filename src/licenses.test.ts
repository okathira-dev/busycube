...
const licenseRoot = new URL("../public/licenses/", import.meta.url);
...
function dependencyLicense(name: string) {
  const regular = new URL(`../node_modules/${name}/LICENSE`, import.meta.url);
  if (existsSync(fileURLToPath(regular))) return regular;
  // Codex's hermetic workspace keeps installed packages under .ignored.
  return new URL(`../node_modules/.ignored/${name}/LICENSE`, import.meta.url);
}

describe("Busycube third-party license distribution", () => {
  it.each([
