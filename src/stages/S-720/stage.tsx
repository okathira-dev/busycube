...
function S720Stage(props: Props) {
  ...
  const [status, setStatus] = useState(() =>
    stageText(props.locale, locale.connectPrompt),
  );
  ...
  useEffect(() => {
    ...
    if (!details) {
      ...
      setStatus(stageText(props.locale, locale.connectPrompt));
      return;
    }
    ...
    if (details.transforms.length === 0) {
      ...
      setStatus(stageText(props.locale, locale.direct));
    } else {
      setStatus(stageText(props.locale, locale.applying));
      void renderVideoRoute(
        ...
        .catch((error: unknown) => {
          if (!disposed && (error as Error).name !== "AbortError") {
            setStatus(
              `${stageText(props.locale, locale.failed)}: ${error instanceof Error ? error.message : stageText(props.locale, locale.unknownError)}`,
            );
          }
        });
    }
  ...
  return (
    <div className="puzzle patchbay-puzzle">
      ...
      <section
        ...
        aria-label={stageText(props.locale, locale.patchBay)}
      >
        <div className="video-patchbay__surface">
          <canvas
            ...
            height="650"
          >
            {stageText(props.locale, locale.canvasDescription)}
          </canvas>
          {sourceNodes.map((id, index) => (
            ...
            >
              <video
                ...
                aria-label={`${stageText(props.locale, locale.sourceAriaPrefix)} ${index + 1}`}
              >
                <track
                  kind="captions"
                  src="data:text/vtt,WEBVTT"
                  srcLang="en"
                  label={stageText(props.locale, locale.silentFixture)}
                />
              </video>
              <span>
                {stageText(props.locale, locale.sourcePrefix).toUpperCase()}{" "}
                {index + 1}
              </span>
              <button
                type="button"
                ...
                aria-label={`${stageText(props.locale, locale.connectSourceOutput)} ${index + 1}`}
              />
            </section>
          ))}
            ...
            transformNodesForColumn(column).map((id, index) => (
              ...
              >
                <button
                  ...
                  aria-label={`${stageText(props.locale, locale.connectNodeInput)} ${id}`}
                />
                ...
                <button
                  ...
                  className={`patch-port patch-port--out${pendingFrom === id ? " patch-port--active" : ""}`}
                  onClick={() => setPendingFrom(id)}
                  aria-label={`${stageText(props.locale, locale.connectNodeOutput)} ${id}`}
                />
              </section>
            )),
          )}
          <section className="patch-node patch-node--output">
            <button
              ...
              aria-label={stageText(props.locale, locale.connectOutput)}
            />
            <span>{stageText(props.locale, locale.outputLabel)}</span>
            {outputUrl ? (
              <video
                key={outputUrl}
                muted
                ...
                controls
                src={outputUrl}
                aria-label={stageText(props.locale, locale.output)}
              >
                <track
                  kind="captions"
                  src="data:text/vtt,WEBVTT"
                  srcLang="en"
                  label={stageText(props.locale, locale.silentOutput)}
                />
              </video>
            ) : (
              <div
                ...
                role="img"
                aria-label={stageText(props.locale, locale.outputIdle)}
              >
                ∿
              </div>
            )}
      ...
      <div className="patchbay-controls">
        ...
        >
          {stageText(props.locale, locale.disconnect)}
        </button>
        <label className="parallel-answer">
          {stageText(props.locale, locale.flagLabel)}
          <input
            value={answer}
            placeholder="busycube{…}"
            onChange={(event) => {
