...
if (!ffmpeg) {
  throw new Error(
    "Set BUSYCUBE_FFMPEG_PATH to a trusted local FFmpeg executable.",
  );
}

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const assetRoot = resolve(root, "src/fixtures/video-recovery/assets");
const temporaryRoot = await mkdtemp(join(tmpdir(), "busycube-poc-022-"));
const frameRate = 12;
const frameCount = 24;
const qrVersion = 2;
const qrSize = 25;
...
const qrTexts = {
  t1: "busycube{swap_halves}",
  t2: "busycube{merge_frames}",
  alpha: "busycube{odd_even_alpha}",
  beta: "busycube{swap_route_beta}",
};
