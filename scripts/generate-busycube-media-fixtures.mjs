...
const ffprobe = process.env.BUSYCUBE_FFPROBE_PATH;
if (!ffmpeg || !ffprobe) {
  throw new Error(
    "Set BUSYCUBE_FFMPEG_PATH and BUSYCUBE_FFPROBE_PATH to trusted local executables.",
  );
}

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const assetRoot = resolve(root, "src/fixtures/media/assets");
const temporaryRoot = await mkdtemp(join(tmpdir(), "busycube-poc-006-"));
...
try {
  ...
  outputs.push(
    await generate("vfr-cadence.webm", [
      ...
      "4",
      "-metadata",
      "title=Busycube POC-006 VFR cadence",
    ]),
  );
  ...
  for (const [name, size, bitrate] of [
    ...
    outputs.push(
      await generate(name, [
        ...
        "4",
        "-metadata",
        `title=Busycube POC-006 ${size} reel`,
      ]),
    );
  }
