...
const databaseName = "busycube-poc-offline-beacon";
...
function receiverDocument() {
  return `<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Busycube offline Beacon receiver</title><main><h1>Offline Beacon receiver</h1><output id="receipt-status" aria-live="polite">worker receiptを照会中</output><p><a href="../">PoC一覧へ戻る</a></p></main><script>const output=document.querySelector('#receipt-status'),nonce=new URL(location.href).searchParams.get('nonce');let attempts=0;const ask=()=>{attempts+=1;navigator.serviceWorker.controller?.postMessage({type:'read-receipt',nonce})};const timer=setInterval(ask,100);navigator.serviceWorker.addEventListener('message',event=>{output.value=event.data.status;if(event.data.found||attempts>=50)clearInterval(timer)});ask();</script></html>`;
}
