...
- Local isolated page: `http://127.0.0.1:4173/poc/`（製品stageから独立）。[probe source](../src/poc/main.ts) と [page](../src/poc/index.html) をGit管理する。
...
- [fixture定義](../src/fixtures/media/fixtures.ts)、[生成script](../scripts/generate-busycube-media-fixtures.mjs)、事前生成したWebM / MP4 / VTT、ffprobeの[generation manifest](../src/fixtures/media/assets/generation-manifest.json)をGit管理する。再生成時は[検証script](../scripts/verify-busycube-media-assets.mjs)でcodec、native寸法、3音声のlabel / language / defaultと全assetの意味契約を照合する。
