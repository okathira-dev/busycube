...
主進捗は IndexedDB の `busycube-progress-v1` データベース、`documents/current` に保存する。保存するのはステージ単位の解決済み箱ID、ステージ内の最小限のmarker、言語設定、インストールIDだけである。クリア日時、判定facts、観測ログ、生入力は保存しない。
...
- 取込み: 選択したJSONを同じschema検証へ通し、解決済み箱IDとmarkerの追加数を確認画面に示してから現在の進捗へマージする。取込み元の `installationId` と言語設定は適用せず、この端末の値を保つ。任意JSONを現在の進捗へ直接代入せず、壊れた文書と将来versionは拒否する。
