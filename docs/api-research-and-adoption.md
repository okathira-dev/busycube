...
API自体は2025-10-17のPrivacy Sandbox整理後も継続サポート対象だが、本番利用にはissuer登録、key commitment、token issuance、redemption endpoint、暗号鍵の保護・rotation・監視を含む独自server-side stackが必要である。[公式developer guide](https://privacysandbox.google.com/protections/private-state-tokens/developer-guide)も独自issuer / redeemer serverの構築を前提にする。Busycubeの架空儀式を任意条件にできる一般向けmanaged issuerは確認できず、`privatetokens.dev`等のdemoをproduction gameplayへ依存させない。DR-131は新規stage、統合、demo依存、historical exhibitを却下し、計画値は78stage・184箱のままとする。
