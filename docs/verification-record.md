...
| 検証 | 結果 | 証跡 |
| --- | --- | --- |
| S-710固定動画 | 合格 | `decode-failure-output.webm`を再生成し、先頭frameを抽出して`busycube{broken_input}`が小文字であることを目視確認 |
...
| Biome | 合格 | `src` と変更したfixture生成script |
| Markuplint | 合格 | `src/**/*.{jsx,tsx,html}` |
...
| S-710 Windows Chrome | 合格 | Git管理QR fixtureでQR frame置換を確認し、lowercase `busycube{qr_replaced}`で開箱。JSON file入力でdecode失敗固定outputと`busycube{broken_input}`を確認 |
| S-720 Windows Chrome / fixture QR | 合格 | `VIDEO 1 → T1 → OUTPUT`の実変換後にlowercase `busycube{swap_halves}`を入力し、T1箱を開箱。FFmpeg抽出frameをjsQRで4 routeとも `busycube{...}`へ照合 |
| 固定media再生成 | 合格 | ローカルのWebM対応FFmpegを`BUSYCUBE_FFMPEG_PATH`へ明示してS-710 decode失敗output、S-720全9本、S-810の120 segment packを再生成。manifest、WebM header、lowercase QR payloadの正本を更新 |
...
| Markdownリンク | 合格 | `src`配下の全相対Markdownリンクが実在 |
...
| 項目 | 結果 | 証跡 |
| --- | --- | --- |
| 開発モード | 合格 | `?mode=development` workerはfetchへ介入せず、即時activate時に旧 `busycube-` キャッシュを削除 |
| 本番HTML | 合格 | Busycubeのnavigation、manifest、iconをnetwork-firstで更新し、正規化した `index.html` をオフラインfallbackに使用 |
...
| TypeScript / Biome | 合格 | `tsc --noEmit`、Busycube 39ファイル |
...
| --- | --- | --- |
| TypeScript | 合格 | `tsc --noEmit` |
| Biome | 合格 | Busycubeの38ファイルを確認 |
| markuplint | 合格 | Busycube配下を確認 |
...
| --- | --- | --- |
| TypeScript | 合格 | `tsc --noEmit` |
| Biome | 合格 | Busycubeコードと `publish-pages.yml` を確認 |
| markuplint | 合格 | Busycube配下を確認 |
| Jest | 合格 | 14 suites / 92 tests。全15ステージ・19問題箱のID対応、入場状態導出、ステージ別累積数を含む |
| production build | 合格 | Vite production buildでBusycube本体と遅延stage chunkを生成 |
...
- リポジトリ: `okathira-dev/busycube`
- ブランチ: `main`
- URL: Viteローカルサーバーの `/index.html`
...
| --- | --- | --- |
| TypeScript | 合格 | `tsc --noEmit` |
| Biome | 合格 | Busycube変更に新規警告なし。リポジトリ既存の設定version情報と `jest.setup.ts` 1警告は継続 |
...
| Biome | 合格 | 追加・変更したBusycube TS/TSX/JS 25ファイルをpass。既存全体警告は別管理 |
...
| media fixture | 合格 | `src/fixtures/media/`へ製品昇格。codec・寸法・音声trackとportable manifestを`verify-busycube-media-assets.mjs`で照合 |
...
| fixture製品昇格 | 合格 | encoding、Unicode font、S-720のsource / intermediate / 4 output、S-710のdecode失敗input / outputを`src/fixtures/`でGit管理。生成script、manifest、形式・寸法・内容の意味検証を同期し、製品stageのPoC path参照0件 |
...
| S-780 | B01〜B04実装済み・公開origin人手待ち | 架空BCU method manifest、○/◇の2 app manifestとworker scope、共有handler runtimeを製品assetへ昇格。✓ / × / ↻→✓はwallet非依存、◇選択は対象workerのtrusted eventだけで開く。fixture testで2 app構造を固定し、H-050でbrowser chooserを確認する |
...
| markuplint | 合格 | `src/**/*.{jsx,tsx,html}` 全対象 |
| Biome | 合格 | 追加・変更Busycube sourceを`biome check` |
| Jest | 合格 | 全50 suites / 301 tests。stage catalogue、metadata locale、S-810関数、S-780の2 Payment App fixtureと既存fixture検証を含む |
| Vite production build | 合格 | stage lazy chunks、`dist/payment/`、`dist/presentation-receiver.html`を生成 |
