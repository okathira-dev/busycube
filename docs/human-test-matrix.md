...
| H-050 | Payment Handler架空決済 | header ruleを設定した公開HTTPS originで架空method manifestが○/◇の2 Payment Appを列挙し、別scopeのService Worker登録、browser所有chooser、各handler windowを確認する。B01はどちらの財布でも承認responseと`complete("success")`、B02は意図的拒否responseと`complete("fail")`、B03は実`retry()`後の同一handler二度目の成功、B04はchooserで◇を選んでそのworkerへtrusted `PaymentRequestEvent`が届いた時だけ開くことを確認する。B04ではその後の✓／×／↻を限定せず、○選択、page click、偽message、stale request ID、game製picker、登録済み判定、`canMakePayment()`だけでは開かない。旧単一walletだけが残らないこと、cancel、例外、late / duplicate response、reset、離脱も確認する。通常のGitHub Pagesではなくmethod URLへ`Link: rel=payment-method-manifest`を返せるmanaged static hostを使い、payer / shipping情報、実payment method、credential、架空response detailsを保存・同期・外部送信しないことも確認する | G-077 | 必須・B01〜B03 PoC確認済み、B04と公開origin確認待ち |
