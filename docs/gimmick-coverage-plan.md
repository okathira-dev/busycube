...
| G-006 Clipboard API | S-180 見えない受け渡し | 再設計承認済み。copy操作で`ebucysub`を書き、playerがpage外で`busycube`へ直して再copyし、箱click時の`clipboard.readText()`が完全一致すると開く1箱。現行の単純write / paste 2箱はこの往復条件へ統合する |
...
| G-021 Web Share / Web Share Target | S-240 渡した印 | 現行B01は実装済み。targetまたはOSへのpayload引き渡しを送出箱とし、installed Busycubeが同じround URLを受信するB02を追加する。PWAインストール導線を含め承認済み・未実装 |
...
| G-033 | S-350 映像の手触り | HTMLMediaElement controls / playbackRate / media tracks / Picture-in-Picture | B01〜B06を実装。native seek、mute / volume 0、再生後の終了前pause、1倍速以外への`ratechange`、native字幕label `Busycube`、PiP入場を独立観測する。Media Capabilities profile箱はD-141、実寸reelはD-142で不採用。frame cadenceはG-080 / S-810へ分離。音声trackは対応環境でのみ将来B07 |
...
| G-042 | S-440 ファイルの鍵（仮） | File Handling / LaunchQueue / FileSystemFileHandle | 採用。round情報を含む`.busycube`をdownloadし、OSの「開く」からinstalled PWAへ渡された実file handleの内容が一致した場合に開く1箱。通常dropは判定外 |
| G-043 | S-450 別の名前で呼ぶ（仮） | Protocol Handlers / LaunchQueue | 採用。stageで発行したround nonce入り`web+busycube:` URLをuser activationで開き、registered PWAがhandler targetを受けた場合に開く1箱 |
...
| G-047 | S-490 名前の鍵（仮） | HTML input / InputEvent | 採用。placeholderだけで`busycube`を示し、text inputの現在値が小文字で完全一致すると開く1箱。入力方法は限定せず、値や入力履歴は保存しない |
| G-048 | S-500 暗号の紙片（仮） | Clipboard Events / Selection | 採用。Caesar暗号文全体のtrusted copyで平文をclipboardへ入れ、trusted pasteで紙面へ戻し、そのDOM内の`busycube`だけを選ぶと開く1箱。S-180、S-490とは別stage |
...
| G-057 | S-580 声の鍵（仮） | Web Speech: SpeechRecognition / SpeechSynthesis | B01/B02を初版実装。B02は入力のi文字目を+i shiftし、変換結果を表示せず発話する。`aspuxouw → busycube`のutteranceがstart後、取消・errorなしでendした時に開く |
