...
- 製品stageは検証済みfixtureの境界を通して参照する。Unicode / encodingは`src/fixtures/**`のproduct facade、media / videoも同じ製品fixture領域を正本として使う。
...
- S-580-B02は`aspuxouw`相当の変換後文字列を画面へ出さず、一文字ずつqueueして全utterance正常終了で開く。
...
- B04: Busycube生成metadataの実照合後、全frameへ固定flagをoverlayする。filenameやMIMEだけでは開かない。
