...
> この表の末尾状態語は着手時点の履歴である。現在状態は[ステージ実装状況](./stage-implementation-status.md)、解法は[現行ステージ解法仕様](./stage-walkthroughs.md)を正とする。G-060〜G-079の追加履歴に加え、第16段階にD-140でS-350のframe cadenceをG-080 / S-810へ分離した。
...
| G-006 | Clipboard API | copy操作で逆順の`ebucysub`をclipboardへ入れ、外で`busycube`へ直して再copyした状態で箱を押す | clipboardへ書いた不完全な鍵をpage外で修復し、箱clickのuser activationで読み戻す | Webページ基盤 + 遷移 | H-001, H-002, H-003, H-004, H-006, H-014, H-020, H-025 | 採用・S-180再設計待ち |
...
| G-021 | Web Share / Web Share Target | 一時tokenを外へ渡す箱と、installed Busycubeを共有先にして同じroundを外から受け取る箱を分ける | Webが共有の送信元と受信先の両方になる | 遷移 + PWA + 環境依存 | H-005, H-014, H-004, H-023 | 採用・S-240再設計待ち |
...
| G-033 | HTMLMediaElement controls / media tracks | B01〜B03はnative seek、mute、play後pause。B04は1倍速以外へのnative速度変更、B05は`Busycube`字幕track、B06はnative PiP入場。対応環境限定の`Busycube`音声trackは将来B07 | UA controlsとnative track選択が実media状態へ反映される挙動を観測する | Webページ基盤 + 端末 + 環境依存 | H-001, H-002, H-003, H-012, H-019, H-020, H-023, H-025, H-030, H-052 | B01〜B06実装済み。Media Capabilities profile箱と実寸reelは不採用、frame cadenceはG-080 / S-810、PiPはG-020から統合 |
...
| G-042 | File Handling / LaunchQueue | stageから得た固有拡張子の鍵fileをOSの「開く」でinstalled Busycubeへ渡す | file managerがPWAの入口になり、実file handleが起動payloadになる | 保存 + 遷移 + PWA + 環境依存 | H-005, H-006, H-019, H-021, H-023, H-025 | 採用・S-440技術スパイク待ち |
| G-043 | Protocol Handlers / LaunchQueue | round固有値を含む`web+busycube:` linkからinstalled Busycubeを起動する | HTTPSとは別のURL schemeがOS / browserのapp routingを通る | 遷移 + PWA + 環境依存 | H-005, H-006, H-019, H-021, H-023, H-025 | 採用・S-450技術スパイク待ち |
...
| G-047 | HTML input / InputEvent | placeholderの`busycube`を手掛かりに、同じ小文字列をtext inputへ完全一致で入れる | 後続の暗号問題で鍵になる語を、先行stageの弱い記憶として残す | Webページ基盤 | H-001, H-002, H-003, H-004, H-020, H-025 | 採用・S-490計画 |
| G-048 | Clipboard Events / Selection | Caesar暗号文の全体copyで表示と異なる平文を入れ、実paste後に平文中の`busycube`だけを選ぶ | copy overrideで初めて現れる平文と、最後のSelection範囲が一つの暗号chainになる | Webページ基盤 + 遷移 | H-001, H-002, H-003, H-004, H-006, H-014, H-020, H-025 | 採用・S-500計画 |
| G-049 | HTML Drag and Drop / DataTransfer / File / `text/uri-list` / `window.open` | B01はページ内PNGをURIで受ける。B02はdraggable=false画像を保存してOS Fileで受ける。B03はiframe画像を拒否し、別windowのPNG URIを受ける | diskやserverを判定経路にせず、ページ、OS、別windowのDrag Data Store境界をプレイヤーがcursorと実dropで見分ける | 遷移 + desktop browser + 環境依存 | H-001, H-002, H-003, H-005, H-013, H-014, H-019, H-020, H-023, H-025 | B01〜B03実装済み・fixtureと人手確認待ち |
...
| G-057 | Web Speech: SpeechRecognition / SpeechSynthesis | B01はone-shot認識のfinal resultが`busycube`なら開く。DR-063のB02は入力文字を位置ごとに+1、+2…shiftし、画面に出さず一文字ずつ読み上げ、`aspuxouw → busycube`の正常発話完了で開く | browserのspeech-to-text入力と動的text-to-speech出力を別箱にし、B02は聞こえた変換列だけから規則を推測する | 端末 + 権限 + Labs + 環境依存 | H-006, H-007, H-019, H-020, H-023, H-025, H-027 | B01 / B02初版実装済み・人手確認待ち |
...
| G-068 | 断片をたどる文書 | 同一page内のText Fragment linkを順に辿り、browserが示した複数の一節からhintを集めて最終回答を作る | URLそのものを長文内の移動手段にし、scriptから読めないUA highlightを手掛かりとして使う | Labs + browser依存 | H-054 | S-690製品実装済み・人手確認待ち。4 jumpをscriptで数えず、`busycube{text_fragments_leave_trails}`の完全一致だけで開く |
| G-069 | 遠くの映写箱 | B01 / B02はRemote Playback接続中の外部画面へ動画内の文字鍵またはround別QRを映し、手元pageで文字入力または実`BarcodeDetector`によるcamera読取を行う。B03はPresentation receiverを外部画面へ実表示する | 現在はB03だけをS-700へ実装。B01 / B02はRemote Playback環境待ちの候補として分離し、B03はmessage列パズルにしない | 端末 + 外部機器 + 権限 + Labs + 環境依存 | H-003, H-004, H-006, H-019, H-020, H-023, H-025, H-040, H-041 | B03製品実装済み・H-041待ち。B01 / B02は未実装 |
...
| G-077 | 四つの財布 | 架空payment handlerで承認、意図的拒否、同一handler再試行を成立させ、browser所有chooserで◇walletも選ぶ | browser所有の2 handler候補、trusted PaymentRequestEvent、handler window、complete / retryを4箱へ分ける。B01〜B03はwallet非依存、B04は◇workerへの実到達 | Labs + Chromium系 + HTTPS + Service Worker | H-003, H-004, H-019, H-023, H-025, H-050 | D-147で4箱へ改訂。製品stage実装済み・H-050待ち。結果label、完了message、固定flagを表示しない |
| G-078 | 活字の鍵 | Git管理するBusycube専用fontをOSへinstallし、Local Font Accessでその実font dataを再発見する | OSのfont install UIとbrowserのlocal-fonts許可を通り、system fontになったfixtureだけがWebへ戻る | Labs + desktop Chromium + OS変更 + 権限 | H-003, H-004, H-006, H-014, H-019, H-023, H-025, H-051 | 採用・S-790未実装。全fontを列挙せず対象PostScript名だけを要求し、端末固有font名・固定flagを使わない |
| G-079 | 断片を組み立てる文書 | 同一pageの英文に対してplayer自身がText Fragment付きURLを組み立て、指定された単語だけをbrowserにhighlightさせる | fragmentを読む箱と単語からfragmentを作る箱を対にし、address barを入力面、UA highlightを結果表示にする | Labs + Text Fragment / `hidden=until-found`対応browser | H-055 | 採用・S-800実装済み。B01 / B02の2箱。入力欄なし。`beforematch`を判定に使い、UA highlightとCtrl+F起点の制約はH-055で確認する |
